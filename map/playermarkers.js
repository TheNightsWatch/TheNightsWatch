 ...
 */
function createInfoWindow(name,data) {
    var html        = "<div class=\"infoWindow\" style='width: 300px'><img src='"+getAvatarURL(name)+"'/><h1>"+name+"</h1><br />Loc: ("+data.x+","+data.z+")</div>";
...
function loadPlayersSuccess(data) {
            for (var i in data) {
                ...
                var server			=	 smartServerName(item.server);
                var rank			=	 item.rank;
                ...
                //    var infoWindow  =    createInfoWindow(name,data[i]); //create the info window
                //    var listener    =    createInfoWindowListener(marker,infoWindow); //create the listener on the marker for the info window
                    var listing     =    createPlayerListing(playerListElement,name,icon,data[i]); //create the player listing
           ...
           //             listener:       listener, //The map marker listener
                ...
                playerMarkers[name].server		= 	 server;
                playerMarkers[name].rank		=	 rank;
